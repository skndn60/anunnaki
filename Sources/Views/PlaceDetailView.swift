...
struct PlaceDetailView: View {
    ...
    private var relatedEvents: [Event] {
        events.filter { $0.place?.persistentModelID == place.persistentModelID }
    }
                            ...
                            ForEach(relatedFigures) { figure in
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 4)
                                        .fill(figureColor(figure.figureType).opacity(0.1))
                                )
                            }
                        }
                    }
    ...
    private func eventColor(_ type: Event.EventType) -> Color { type.color }
