    ...
    private func serviceRow(label: String, figures: [Figure], color: Color) -> some View {
        HStack(spacing: 8) {
            ...
            ForEach(figures) { fig in
                MiniChip(name: fig.name, symbol: fig.gender.symbol, color: fig.figureType.color, isClickable: true) {
                    recenter(on: fig)
                }
                .contextMenu {
    ...
    private func centerRow(_ center: Figure) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                .fill(center.figureType.color.opacity(0.06))
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(center.figureType.color.opacity(0.15), lineWidth: 1)
                )
        )
    }
