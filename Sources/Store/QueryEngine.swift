...
class QueryEngine {
    ...
    func query(_ input: String) -> QueryResult {
        ...
        // Try possessive patterns for events: "Great Flood's figures", "Great Flood's place"
        if let eventName = extractPossessivePattern(text, suffixes: ["figures", "place", "location"]) {
            if let event = resolveEvent(eventName) {
                let isPlaceQuery = text.contains("'s place") || text.contains("'s location")
                if isPlaceQuery {
        ...
        if let eventName = extractPattern(text, patterns: ["place of ", "location of "]) {
            if let event = resolveEvent(eventName) {
                if let place = event.place {
                    return .place(context.buildPlaceDossier(place))
                }
    ...
    private func findEvents(byPlaceName placeName: String) -> [Event] {
        let events = context.fetchAll() as [Event]
        return events.filter { $0.place?.name.lowercased() == placeName.lowercased() }
    }
