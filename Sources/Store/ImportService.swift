...
struct ImportService {
    ...
    func fetchWikidata(title: String) async -> (ParsedWikidata, [String])? {
        ...
        var details: [String] = []
        if let ft = parsed.figureType { details.append("type: \(ft.rawValue)") }
    ...
    func matchFigure(query: String, in figures: [Figure]) -> Figure? {
    ...
    func matchPlace(query: String, in places: [Place]) -> Place? {
        let mq = query.lowercased()
    ...
    func matchEvent(query: String, in events: [Event]) -> Event? {
        let mq = query.lowercased()
    ...
    func applyToFigure(_ figure: Figure, parsed: ParsedWikidata, extract: String, wikiURL: String, allFigures: [Figure]) {
        if let ft = parsed.figureType, figure.figureType == .deity { figure.figureType = ft }
    ...
    func applyToPlace(_ place: Place, parsed: ParsedWikidata, extract: String, wikiURL: String) {
        if let pt = parsed.placeType { place.placeType = pt }
        if place.placeDescription.isEmpty { place.placeDescription = extract }
        place.source = wikiURL
    }

    func applyToEvent(_ event: Event, parsed: ParsedWikidata, extract: String, wikiURL: String) {
        if let et = parsed.eventType { event.eventType = et }
    ...
    @discardableResult
    func createCitation(sourceTitle: String, wikiURL: String, extract: String, entityName: String, entityType: Citation.EntityType) -> Citation {
        let source = Source(
            name: sourceTitle,
            sourceType: .other,
            author: "Wikipedia",
    ...
    func createStandaloneSource(title: String, extract: String, wikiURL: String) {
        let source = Source(
            name: title,
            sourceType: .other,
            author: "Wikipedia",
